...
bool TSPIndividual::isValid()
{
    ...
    bool result = true;
    vector<int> route = getSolution();
    unsigned int n_cities = m_problem->get_n_cities();
...
{
    int dim = m_rnd->Rannyu(0, m_gene.size()-1);
    int i_start = m_rnd->Rannyu(0, m_gene.size()-dim);
    int n_shift = m_rnd->Rannyu(0, m_gene.size()-i_start-dim);

    rotate(m_gene.begin()+i_start, m_gene.begin()+i_start+dim, m_gene.begin()+i_start+dim+n_shift);
