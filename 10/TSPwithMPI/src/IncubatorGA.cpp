...
p_m(p_mut)
{
    ...
    for(int i=0;i<individuals_for_generation;i++)
...
{
    TSPIndividual firstInd, secondInd;
    double fr, sr;

    for(int i_gen=0;i_gen<n_generations;i_gen++)
    {
        #ifdef VERBOSE
        ...
        while(new_generation.size() < individuals_for_generation)
        {
            ...
            firstInd = selectIndividual();
            secondInd = selectIndividual();
            
            if(m_rnd.Rannyu()<p_c)               
                firstInd.Crossoover(secondInd);

            new_generation.push_back(firstInd);
            new_generation.push_back(secondInd);
        }
        ...
        if(saveLoss)
        {
            for(int i_ind=0;i_ind<current_generation.size(); i_ind++)
            {
...
TSPIndividual IncubatorGA::selectIndividual()
{
    return current_generation.at(current_generation.size()*pow(m_rnd.Rannyu(), q));
}

void IncubatorGA::injectIndividual(TSPIndividual individual)
{
